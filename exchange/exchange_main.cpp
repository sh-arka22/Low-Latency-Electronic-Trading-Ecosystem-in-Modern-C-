...
int main(int, char **) {
  ...
  const int sleep_time = 100 * 1000;  // 100 ms in microseconds * 1000 below.

  // The three LFQueues that wire MatchingEngine to OrderServer / MarketDataPublisher.
  ...
  // 2. Market data publisher: snapshot on 233.252.14.1:20000, incremental on 233.252.14.3:20001.
  logger->log("%:% %() % Starting Market Data Publisher...\n",
              __FILE__, __LINE__, __FUNCTION__,
              Common::getCurrentTimeStr(&time_str));
  const std::string mkt_pub_iface = "lo0";  // macOS loopback; book uses "lo"
  ...
  order_server = new Exchange::OrderServer(
      &client_requests, &client_responses, order_gw_iface, order_gw_port);
  order_server->start();
