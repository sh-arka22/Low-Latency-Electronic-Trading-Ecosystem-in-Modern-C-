...
namespace Exchange {
  MarketDataPublisher::MarketDataPublisher(MEMarketUpdateLFQueue *market_updates,
    ...
    ASSERT(incremental_socket_.init(incremental_ip, iface, incremental_port,
    ...
    while (run_) {
      for (auto market_update = outgoing_md_updates_->getNextToRead();
        ...
        logger_.log("%:% %() % Sending seq:% %\n", __FILE__, __LINE__,
                    ...
                    next_inc_seq_num_, market_update->toString());

        // Assemble MDPMarketUpdate on the wire as two staged sends.
        ...
        // Forward to snapshot synthesizer with the same seq number.
        auto *slot = snapshot_md_updates_.getNextToWriteTo();
        slot->seq_num_           = next_inc_seq_num_;
        slot->me_market_update_  = *market_update;
        snapshot_md_updates_.updateWriteIndex();

        ++next_inc_seq_num_;
      }

      // One flush per round — batches many sends into one (or few) syscalls.
