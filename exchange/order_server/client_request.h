...
#pragma pack(push, 1)
  ...
  /// Order Gateway Server thread -> Matching Engine thread.
  /// Packed so the same struct can later be wired straight onto the network.
  ...
  /// TCP wire format = per-client sequence number + MEClientRequest.
  /// Lives inside the same pack(1) block so layout is stable across machines.
